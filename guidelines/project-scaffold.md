# 🏗️ Project Scaffold Convention (Hexagonal Architecture)

## 1. 레포지토리 명명 규칙
- 패턴: `{domain}-service`
- 예시: `user-service`, `order-service`, `product-service`
- 각 MSA 서비스는 독립된 Git 레포지토리로 관리한다
...
│   │   │   │   └── error/
│   │   │   │       ├── SystemErrorCode.java               (enum — HTTP 오류)
│   │   │   │       ├── BusinessResultCode.java            (enum — 비즈니스 결과)
│   │   │   │       ├── BusinessException.java
│   │   │   │       └── GlobalExceptionHandler.java
│   │   │   ├── domain/
│   │   │   │   ├── {Domain}.java                          (순수 Java Entity)
│   │   │   │   └── {Domain}Id.java                        (VO, record)
│   │   │   ├── port/
│   │   │   │   ├── in/
│   │   │   │   │   ├── Create{Domain}UseCase.java         (interface)
│   │   │   │   │   └── Find{Domain}UseCase.java           (interface)
│   │   │   │   └── out/
│   │   │   │       └── {Domain}Repository.java            (interface)
│   │   │   ├── application/
│   │   │   │   └── {Domain}Service.java                   (implements UseCase 인터페이스들)
│   │   │   └── adapter/
│   │   │       ├── in/
│   │   │       │   └── web/
...
│   │   │           │   ├── {Domain}JpaRepository.java        (Spring Data)
│   │   │           │   └── {Domain}RepositoryAdapter.java    (implements {Domain}Repository)
│   │   │           └── external/                             (의존 있을 때만)
│   │   │               ├── {TargetDomain}Client.java         (Feign Client interface)
│   │   │               └── {TargetDomain}ClientAdapter.java  (implements {TargetDomain}Port)
│   │   └── resources/
│   │       └── application.yml
│   └── test/
│       └── java/com/{프로젝트명}/{domain}/
...
### UseCase 인터페이스 예시
```java
package com.{프로젝트명}.{domain}.port.in;
...
### Service (UseCase 구현체)
```java
package com.{프로젝트명}.{domain}.application;
...
public class UserService implements CreateUserUseCase, FindUserUseCase {
    ...
    private final UserRepository userRepository; // port/out 인터페이스에 의존
...
### RepositoryAdapter (port/out 구현체)
```java
package com.{프로젝트명}.{domain}.adapter.out.persistence;
...
public class BusinessException extends RuntimeException {
    private final BusinessResultCode resultCode;
    ...
    public BusinessResultCode getResultCode() { return resultCode; }
}
```
